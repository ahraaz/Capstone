...
<?php 
	...
	if(isset($_GET['del']))
	{
		...
		if($count == 1)
		{
			if($del == 0)
			{
				$sql = "DELETE FROM groups WHERE email = (SELECT username FROM users WHERE id = $id[0]);";
			...
			{
				$sql = "DELETE FROM projects WHERE id = $id[0];";
				mysqli_query($link, $sql);
				
				$sql = "DELETE FROM groups WHERE project = $id[0];";
				mysqli_query($link, $sql);
			}
			else if($del == 2)
			{
				$sql = "DELETE FROM suggestions WHERE id = $id[0];";
				mysqli_query($link, $sql);
			}
			else if($del == 3)
			{
				$sql = "DELETE FROM messages WHERE id = $id[0];";
				mysqli_query($link, $sql);
			}
		}
		// delete a range of ids
		else
		{
			if($del == 0)
			{
				$sql = "DELETE FROM groups WHERE email = (SELECT username FROM users WHERE id >= $id[0] AND id <= $id[1]);";
				mysqli_query($link, $sql);
				
				$sql = "DELETE FROM users WHERE id >= $id[0] AND id <= $id[1];";
				mysqli_query($link, $sql);
			}
			else if($del == 1)
			{
				$sql = "DELETE FROM projects WHERE id >= $id[0] AND id <= $id[1];";
				mysqli_query($link, $sql);
				
				$sql = "DELETE FROM groups WHERE project >= $id[0] AND project <= $id[1];";
				mysqli_query($link, $sql);
			}
			else if($del == 2)
			{
				$sql = "DELETE FROM suggestions WHERE id >= $id[0] AND id <= $id[1];";
				mysqli_query($link, $sql);
			}
			else if($del == 3)
			{
				$sql = "DELETE FROM messages WHERE id >= $id[0] AND id <= $id[1];";
				mysqli_query($link, $sql);
			}
		}
	}
...
<section class="header4 cid-rlYqeIBrl5" id="header4-1q">
	<div class="container">
		...
		<ul class="nav justify-content-center nav-pills">
			<li class="nav-item">
				<a class="nav-link <?php if($submenu == 0) echo active ?>" href="delete.php?menu=0">Users</a>
			</li>
			<li class="nav-item">
				<a class="nav-link <?php if($submenu == 1) echo active ?>" href="delete.php?menu=1">Projects</a>
			</li>
			<li class="nav-item">
				<a class="nav-link <?php if($submenu == 2) echo active ?>" href="delete.php?menu=2">Suggestions</a>
			</li>
			<li class="nav-item">
				<a class="nav-link <?php if($submenu == 3) echo active ?>" href="delete.php?menu=3">Messages</a>
			</li>
		</ul>
		<br>
		<ul class="nav justify-content-center nav-pills">
			...
			<li class="nav-item">
				<a class="nav-link active" href="delete.php?menu=<?php echo $submenu?>">Delete</a>
			</li>
			<li class="nav-item">
		...
		<br><br>
		<?php
		// display a form based on what the user wants to add(users, projects, suggestions, messages)
		if($submenu == 0)
		{
			echo "<h2>Delete user:</h2>
			<form method='post' action='delete.php?menu=" . $submenu . "&del=0'>
				<input type='text' name='id' class='form-control' placeholder='id' /><br>
				<p>*Delete a range of ids with '-'</p>
				<p>*Example: 1-5</p><br>
		...
		else if($submenu == 1)
		{
			...
			<form method='post' action='delete.php?menu=" . $submenu . "&del=1'>
				<input type='text' name='id' class='form-control' placeholder='id' /><br>
				<p>*Delete a range of ids with '-'</p>
				<p>*Example: 1-5</p><br>
				<div class='col-md-12 input-group-btn align-center'><input type='submit' name='' class='btn btn-primary btn-form display-3' value='Submit'>
		...
		{
			echo "<h2>Delete suggestion:</h2>
			<form method='post' action='delete.php?menu=" . $submenu . "&del=2'>
				<input type='text' name='id' class='form-control' placeholder='id' /><br>
				<p>*Delete a range of ids with '-'</p>
				<p>*Example: 1-5</p><br>
				<div class='col-md-12 input-group-btn align-center'><input type='submit' name='' class='btn btn-primary btn-form display-3' value='Submit'>
		...
		{
			echo "<h2>Delete message:</h2>
			<form method='post' action='delete.php?menu=" . $submenu . "&del=3'>
				<input type='text' name='id' class='form-control' placeholder='id' /><br>
				<p>*Delete a range of ids with '-'</p>
				<p>*Example: 1-5</p><br>
				<div class='col-md-12 input-group-btn align-center'><input type='submit' name='' class='btn btn-primary btn-form display-3' value='Submit'>
		...
		if(isset($_GET['del']))
		{
			...
			if($count == 1)
			{
				echo "<h3>Successfully deleted id(" . $id[0] . ")!</h3>";
			}
			else
			{
				echo "<h3>Successfully deleted id(" . $id[0] . " - " . $id[1] . ")!</h3>";
			}
